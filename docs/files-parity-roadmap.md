...
| 会话恢复 / 最近关闭标签 | — | ⬜ |
| NavigationToolbar（Tab 下全宽，侧栏之上） | `main_page` `navigation-toolbar` | 🟡 |
| InnerNavigationToolbar（内容区顶） | `file_browser` `content-toolbar` | 🟡 |
| 地址栏 / Omnibar | 面包屑 + 路径编辑 + 右侧搜索（见 gap 文档） | 🟡 |
| 侧栏折叠 | `h_resizable` | ✅ |
| Sidebar 分区结构 | Home 独立 + Places(回收站) + 各分区 | 🟡 |
...
| 多选 Ctrl/Shift | ✅ | ✅ |
| 默认无选中 / 点空白取消选中 | ✅ | ✅ |
| 列表空白区右键（粘贴/新建/刷新） | BaseContextMenuFlyout | 🟡 |
| 键盘快捷键 | `commands` | ✅ |
...
| Widget | 状态 |
|--------|------|
| Widget 子系统 + 设置/右键开关 | H0–H4 | ✅ |
| Quick Access | 卡片网格 + Pin + 右键；缩略图/监听待做 | 🟡 |
| Drives | 卷标 + 容量条 + 右键；Eject/Storage Sense 待做 | 🟡 |
| Network locations | 卡片 + Known Folder | 🟡 |
| File tags | 预览容器 + 跳转标签列表 | 🟡 |
| Recent files | 名 + 路径列 | 🟡 |
...
1. ~~MainPage 骨架~~ ✅
2. ~~Home 小部件（H0–H4 卡片/容量/菜单）~~ 🟡 → 见 `home-files-parity.md` H5+
