...
| 快速枚举 | — | ⬜ |
| 右键菜单（Files 式 Flyout） | GPUI `ContextMenu` + `query_shell_context_menu_items` 合并 | 🟡 |
...
8. ~~回收站虚拟列表~~ 🟡（首轮）
9. **进行中**：Files 式右键菜单（GPUI + Shell 条目合并），见 `files-context-menu-parity.md`
10. **下一步**：OS 级拖放、回收站还原/永久删除、M6 高级功能。
