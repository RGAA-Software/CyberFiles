...
impl ResizablePanel {
    ...
    /// Mark this panel as having a fixed absolute size.
    ///
    /// When the container resizes, fixed panels keep their previous pixel
    /// width; only the flexible siblings are scaled proportionally.
...
impl RenderOnce for ResizablePanel {
    fn render(self, _: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        // Sync fixed_size flag into state so adjust_to_container_size sees it.
        state.update(cx, |state, _cx| {
            if let Some(p) = state.panels.get_mut(self.panel_ix) {
        ...
        div()
            ...
            .when_some(self.initial_size, |this, initial_size| {
                ...
                this.when(
                    panel_state_size.is_none() && !initial_size.is_zero(),
                    |this| this.flex_none(),
                )
                .flex_basis(initial_size)
            })
            .map(|this| match panel_state_size {
