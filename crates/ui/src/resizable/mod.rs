...
impl ResizableState {
    ...
    ///
    /// Panels marked with `fixed_size = true` keep their previous absolute size;
    /// the remaining space is distributed proportionally among the flexible panels.
    fn adjust_to_container_size(&mut self, cx: &mut Context<Self>) {
        ...
        let container_size = self.container_size();
        let old_total = px(self.sizes.iter().map(|s| s.as_f32()).sum::<f32>());
        ...
        for i in 0..self.panels.len() {
            let new_size = if self.panels[i].fixed_size {
                self.sizes[i]
                    .min(self.panel_size_range(i).end)
                    .max(self.panel_size_range(i).start)
            } else {
                let ratio = self.sizes[i] / flex_total;
                let size = remaining * ratio;
                size.min(self.panel_size_range(i).end)
                    .max(self.panel_size_range(i).start)
            };
